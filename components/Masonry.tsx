...
const Masonry = ({
  ...
  const grid = useMemo(() => {
    ...
    const gap = 16;
    const totalGaps = ((columns as number) - 1) * gap;
    const columnWidth = (width - totalGaps) / (columns as number);
  ...
  return (
    <div ref={containerRef} className={`relative w-full ${className}`} style={{height: grid.length > 0 ? Math.max(...grid.map(i => i.y + i.h)) : '100vh'}}>
      {grid.map(item => (
        <div
          key={item.id}
          data-key={item.id}
          ...
          onClick={() => {
            if (onItemClick) onItemClick(item.id, item as MasonryItem);
